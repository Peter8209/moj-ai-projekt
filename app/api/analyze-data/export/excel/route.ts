...
const EXPORT_ROUTE = '/api/analyze-data/export/excel';
...
async function handleExportRequest(request: NextRequest, forcedFormat?: ExportFormat) {
  try {
    ...
    return binaryDownloadResponse(buffer, EXCEL_MIME_TYPE, fileName, {
      ...
      'X-Zedpera-Excel-Charts': renderedBuffer ? 'embedded-single-per-sheet' : 'fallback-none',
      'X-Zedpera-Chart-Limit': '1',
      'X-Zedpera-First-Sheet': '01 overview',
    });
    ...
    console.error('[api/analyze-data/export/excel] Chyba exportu:', error);
