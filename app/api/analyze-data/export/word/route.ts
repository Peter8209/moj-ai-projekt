...
const EXPORT_ROUTE = '/api/analyze-data/export/word';
...
function buildExportHtmlDocument(
  ...
  function compactWordCellValue(value: unknown): string {
  ...
  function getColumnWidthPercent(columnCount: number): string {
  ...
  function buildWordTablePage(params: {
    ...
    const width = getColumnWidthPercent(headers.length);

    const headingSuffix = [
      columnPageCount > 1
        ? `stĺpce ${columnPageIndex + 1}/${columnPageCount}`
        : '',
      rowPageCount > 1
        ? `strana ${rowPageIndex + 1}/${rowPageCount}`
        : '',
    ]
      .filter(Boolean)
      .join(' · ');
    ...
    const head = headers
      .map(
        (header) =>
          `<th scope="col">${htmlEscape(header)}</th>`,
      )
      .join('');
    ...
    const body = rows.length
              ...
              <tr>
                ${headers
                  .map(
                    (header) =>
                      `<td>${htmlEscape(
                        compactWordCellValue(row[header]),
                      )}</td>`,
                  )
                  .join('')}
              </tr>
      ...
      : `
          <tr>
            <td colspan="${Math.max(headers.length, 1)}" class="empty-cell">
              Pre túto tabuľku neboli dostupné údaje.
            </td>
          </tr>
        `;

    return `
      <section class="report-page${firstPage ? ' first-report-page' : ''}">
        <div class="section-header">
          <div class="section-number">${htmlEscape(definition.sheetName)}</div>
          <h2>${htmlEscape(definition.title)}</h2>
          ${
            headingSuffix
              ? `<div class="section-continuation">${htmlEscape(headingSuffix)}</div>`
              : ''
          }
        </div>

        <p class="section-description">
          ${htmlEscape(definition.description)}
        </p>
        ...
        <div class="page-footer">
          ZEDPERA · ${htmlEscape(definition.title)}
        </div>
      </section>
    `;
  }
  ...
  const coverHtml = `
    <section class="cover-page">
      <div class="brand">ZEDPERA</div>
      <h1>Výsledky analýzy dát</h1>
      <p class="subtitle">
        Profesionálny tabuľkový report optimalizovaný pre Microsoft Word
      </p>

      <table class="cover-table" cellspacing="0" cellpadding="0">
        <tbody>
          ${overviewRows
            .map(
              (row) => `
                <tr>
                  <th>${htmlEscape(row.oblast)}</th>
                  <td>${htmlEscape(compactWordCellValue(row.hodnota))}</td>
                </tr>
              `,
            )
            .join('')}
      ...
      <div class="cover-note">
        Každá tabuľka začína na samostatnej strane. Široké tabuľky sa
        automaticky rozdeľujú na čitateľné bloky stĺpcov a dlhé tabuľky
        na pokračovacie strany bez prekrývania obsahu.
      </div>
  ...
  const sectionsHtml = definitions
    .flatMap((definition) => {
      ...
      const fallbackRow = {
        stav: 'bez údajov',
        poznamka:
          'Pre túto tabuľku neboli v aktuálnom výsledku dostupné údaje.',
      };

      const sourceRows =
        normalizedRows.length > 0 ? normalizedRows : [fallbackRow];

      const allHeaders = Array.from(
        new Set(
          sourceRows.flatMap((row) => Object.keys(row)),
        ),
      );
      ...
      return columnChunks.flatMap(
        (headerChunk, columnPageIndex) => {
          const rowChunks = chunkArray(
            sourceRows,
            MAX_ROWS_PER_PAGE,
          );

          return rowChunks.map((rowChunk, rowPageIndex) => {
            const html = buildWordTablePage({
              definition,
              headers: headerChunk,
              rows: rowChunk,
              columnPageIndex,
              columnPageCount: columnChunks.length,
              rowPageIndex,
              rowPageCount: rowChunks.length,
              firstPage: reportPageCounter === 0,
            });
...
<head>
  ...
  <meta name="Generator" content="ZEDPERA" />
  <meta name="Originator" content="ZEDPERA" />
  <title>ZEDPERA – výsledky analýzy dát</title>
  ...
  <style>
    @page WordLandscape {
      ...
      margin: 1.25cm 1.25cm 1.35cm 1.25cm;
    }
...
export async function GET() {
  return jsonResponse({
    ...
    route: EXPORT_ROUTE,
    message:
      'Samostatný Word export ZEDPERA. Endpoint generuje iba Word dokument.',
    usage,
  });
}
...
export async function POST(request: NextRequest) {
  return handleExportRequest(request, 'word');
}
