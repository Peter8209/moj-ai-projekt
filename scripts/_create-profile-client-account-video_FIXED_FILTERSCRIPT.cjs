 ...
 * Verzia: PROFILE_CLIENT_ACCOUNT_PRO_FIXED / 6 JAZYKOV / BEZ AI SPRIEVODCU
 *
 ...
 * - opravené pre Windows: filtergraph sa zapisuje do súboru a neposiela sa ako extrémne dlhý príkaz.
 *
 * Výstup:
...
function renderVideo({ lang, force, seconds, timeout }) {
  ...
  const filterFile = path.join(tmpDir, 'profile-filtergraph.txt');
  ...
  if (result.error) {
    fs.rmSync(tmpFile, { force: true });
    throw new Error(`FFmpeg sa nespustil: ${result.error.code || ''} ${result.error.message || result.error}`);
  }

  if (result.status !== 0) {
    fs.rmSync(tmpFile, { force: true });
    throw new Error(result.stderr || result.stdout || `FFmpeg zlyhal so statusom ${result.status}. Filter script: ${filterFile}`);
  }
